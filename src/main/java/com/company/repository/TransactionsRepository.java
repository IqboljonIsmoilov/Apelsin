...
public interface TransactionsRepository extends JpaRepository<TransactionsEntity, String> {
    ...
    Page<TransactionsMapper> getByCardId(Pageable pageable,
                                         @Param("cid") String cid,
                                         @Param("status") TranStatus status);
    ...
    @Query("SELECT t.id as t_id,fcr.id as fcr_id,fcr.number as fcr_number,fcr.phone as fcr_phone," +
            ...
            " order by t.createdDate")
    Page<TransactionsMapper> getByClientId(Pageable pageable,
                                           @Param("cid") String cid,
                                           @Param("status") TranStatus status);
    ...
    @Query("SELECT t.id as t_id,fcr.id as fcr_id,fcr.number as fcr_number,fcr.phone as fcr_phone," +
            ...
            " order by t.createdDate")
    Page<TransactionsMapper> getByPhone(Pageable pageable,
                                        @Param("phone") String cid,
                                        @Param("status") TranStatus status);
}